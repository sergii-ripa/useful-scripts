...
usage="usage: destroyTenant.sh <tenantId> <platform> <env>"
...
readTenantArgs
echo "About to destroy the tenant with such params: "
echo TENANT_ID=$TENANT_ID
echo PLATFORM=$PLATFORM
echo CREATE_TENANT_ARGS=$CREATE_TENANT_ARGS
...
read -p "Destroying Tenant ${TENANT_ID}? (y/n)" -n 1 -r
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
docker run --rm \
  ...
  $BUILDER_IMAGE destroy

echo "Done destroying ${TENANT_ID} tenant. Check tenant health with this URL: (should fail)"
